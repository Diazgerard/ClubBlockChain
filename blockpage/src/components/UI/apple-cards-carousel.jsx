...
import {IconApple, IconArrowLeft, IconArrowRight, IconX} from "@tabler/icons-react";
        ...
        <CarouselContext.Provider value={{onCardClose: handleCardClose, currentIndex}}>
            <div className="relative w-full">
                ...
                <div className="mr-10 flex justify-end gap-2">
                    <button className="relative z-40 flex h-10 w-10 items-center justify-center rounded-full bg-white/80 border border-white disabled:opacity-50 shadow-lg" onClick={scrollLeft} disabled={!canScrollLeft}>
                        <IconArrowLeft className="h-6 w-6 text-black" />
                    </button>
                    <button className="relative z-40 flex h-10 w-10 items-center justify-center rounded-full bg-white/80 border border-white disabled:opacity-50 shadow-lg" onClick={scrollRight} disabled={!canScrollRight}>
...
export const BlurImage = ({height, width, src, className, alt, ...rest}) => {
    ...
    return (
        <img
            className={cn("h-full w-full transition duration-300 object-contain", isLoading ? "blur-sm" : "blur-0", className)}
